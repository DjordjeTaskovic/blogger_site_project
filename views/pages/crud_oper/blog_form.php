...
if(isset($_POST['blogID'])){
    $headline="<h1>Update blog form</h1>";
    $blogID = $_POST['blogID'];
    ...
    $headline="<h1>Insert blog form</h1>";
...
<div class="col-md-8 offset-2 mt-5">
    <form method="POST" action="<?=$action?>" class="form-contact contact_form"
     id="contactForm" enctype="multipart/form-data">
    <div class="row">
    <div class="col-lg-5">
    <input type="hidden" name="blogid" id='blogid'
     value="<?php if($blogID!=''){echo$blogID;}else{echo'';}?>"/>
    <input type="hidden" name="userid" id='userid'  value="<?=$userid?>"/>

    <div class="form-group">
    <input class="form-control" name="subject" id="subject" type="text" 
    value="<?php if($bloginfo!=''){echo$bloginfo->item_name;}else{echo'';}?>"
    placeholder="Enter blog subject">
    <p class="text-error"></p>
    </div>
        <div class="form-group">
            <select name="cat" id="cat" class="form-control">
            <option value="<?php if($bloginfo!=''){echo$bloginfo->kat_id;}else{echo 0;}?>">
            <?php if($bloginfo!=''){echo$bloginfo->kat;}else{echo'Choose Category';}?></option>
    ...
    placeholder="Enter Message" value="<?php if($bloginfo!=''){echo$bloginfo->info;}else{echo'';}?>">
    <?php if($bloginfo!=''){echo$bloginfo->info;}else{echo'';}?></textarea>
    <p class="text-error"></p>
